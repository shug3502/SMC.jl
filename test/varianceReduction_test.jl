...
###################
function computeLikRatio(th1::Array{Float64}, th2::Array{Float64}, observations::Array{Float64}; N::Int64=64, rho::Float64=0.95)
  numRandoms = size(observations,2)*(N+1)
  u1 = randn(numRandoms)
  u2 = randn(numRandoms)
  u3 = rho*u1 + sqrt(1-rho^2)*u2
  lik1 = runFilter(th1, cdf.(Normal(),u1), observations, x0=x0, N=N)
  ratio1 = exp(lik1 - runFilter(th2, cdf.(Normal(),u2), observations, x0=x0, N=N))
  ratio2 = exp(lik1 - runFilter(th2, cdf.(Normal(),u3), observations, x0=x0, N=N))
...
R = zeros(nRepeats,2)
for i = 1:nRepeats
  R[i,:] = computeLikRatio(th1,th2,observations,N=64)  
end
